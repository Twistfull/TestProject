...
public class ImgEditor { 
		 ...
		 * @return 
		   */
		  public boolean zoomOut(String srcFileName,String tagFileName,double ratio){  
			     try { 
			     BufferedImage bi = ImageIO.read(new File(srcFileName));
			     //int jwidth=bi.getWidth()/(int)ratio; 
			     //int jheight=bi.getHeight()/(int)ratio;
			     int width=(int)(bi.getWidth()/ratio); 
			     int height=(int)(bi.getHeight()/ratio);
			     //System.out.println("原宽:"+jwidth+"原高"+jheight);
			     //System.out.println("宽:"+width+"高"+height);
			     BufferedImage tag=new BufferedImage(width,height, BufferedImage.TYPE_INT_RGB);
			     //默认缩放
			    // tag.getGraphics().drawImage(bi, 0, 0, width, height, null);
			     //区域均值缩放			     
			     tag.getGraphics().drawImage( bi.getScaledInstance(width, height, java.awt.Image.SCALE_AREA_AVERAGING), 0, 0, null); 
			     ImageIO.write(tag, "jpg", new File(tagFileName));//画图 
