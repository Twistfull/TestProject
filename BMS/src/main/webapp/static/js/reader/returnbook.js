...
function productReturnInfo(parm) {  
	...
	        success: function (result) {  
		       if (result &&result.statusCode==1&& result.message){	
		    	   ...
		    	   document.getElementById("returndetail").appendChild(ul);
		    	  ...
		    	  // p1.className="sub-header";
		    	   //document.getElementById("ulid").appendChild(p1);		    	   
	                   ...
	                   if(result.data.returns.payFor<=0){
					       ...
					       li6.appendChild(document.createTextNode("逾期天数:  "+result.data.returns.overdueDay+"  天"));
	                   ...
	                   $("#miss").attr('name',result.data.returns.borrowInfo.bookInfo.bookPrice);
	                   //$('input[name="sb"]')
...
function BookMiss(parm) {
	    	 ...
	    	 // document.getElementById("changediv").appendChild(div);
	    	  ...
	    	  span.id="price";
	    	  document.getElementById("returndetail").appendChild(h3);
	    	  h3.appendChild(document.createTextNode("由于您丢失了图书,很抱歉您需要照价赔偿"));
	    	  document.getElementById("returndetail").appendChild(h4);	    	  
