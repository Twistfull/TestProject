...
$(function() {
	...
	$("#dg").datagrid({
		title:'归还表',
		url : ctx + "/admin/listReturnAction",
		columns : [ [ {
			field : 'recordId',
			title : '记录数',
			width : 100,
			...
			formatter:function(value,row,index){
				return row.borrowPojo.reader.id
			}
		},{
			field : 'readerName',
			title : '读者姓名',
			width : 100,
			formatter:function(value,row,index){
				return row.borrowPojo.reader.readerName
			}
		},{
			field : 'bookId',
			title : '图书编号',
			width : 100,
			formatter:function(value,row,index){
				return row.borrowPojo.bookInfo.bookId
			}
		},{
			field : 'bookName',
			title : '图书名',
			width : 100,
			formatter:function(value,row,index){
				return row.borrowPojo.bookInfo.bookName
			}
		}
		] ],
	...
	*/
	$('#dg').datagrid({
		...
		onExpandRow:function(index,row){
			...
			ddv.datagrid({
				url:ctx + "/admin/listReturnByReaderId?id="+row.id,
				fitColumns:true,
				singleSelect:true,
				rownumbers:true,
				loadMsg:"正在加载",
