		...
		<div class="container-fluid">
			<div class="row">
				...
				<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main" id="changediv">
					...
					<h2 class="sub-header" id="successtip">归还信息</h2>
	                           ...
	                           <li class="media">
	                               ...
	                               <div class="media-body" id="returndetail">
	                                   ...
	                                   <ul class="list-unstyled">
	                                       <li>借阅人:&nbsp;&nbsp;${username}</li>
		                                    <p class="sub-header"></p>
		                                   <li>借阅书籍:&nbsp;&nbsp;${returns.borrowInfo.bookInfo.bookName}</li>
		                                     <p class="sub-header"></p>
		                                   <li>限定日期:&nbsp;&nbsp; 
		                                   <%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt1"%><!-- 格式化日期 -->
										     <fmt1:formatDate value="${returns.borrowInfo.endDate}" pattern="yyyy-MM-dd"/>
		                                   </li>
		                                     <p class="sub-header"></p>
		                                   <li>归还日期:&nbsp;&nbsp;
		                                   <%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt2"%><!-- 格式化日期,注意value里不要有空格 -->
										     <fmt2:formatDate value="${returns.returnDate}" pattern="yyyy-MM-dd"/>
		                                   </li>		                                   
		                                   <p class="sub-header"></p>
		                                   <c:choose>
		                                       <c:when test="${returns.payFor<=0}">
		                                          <li>处于限定时间内归还!&nbsp;&nbsp;</li>
		                                     <p class="sub-header"></p>	
		                                       </c:when>
		                                       <c:otherwise>
		                                          <li>逾期天数:&nbsp;&nbsp;${returns.overdueDay}天</li>
		                                          <p class="sub-header"></p>		                                   
		                                          <li>罚款金额:&nbsp;&nbsp;${returns.payFor}元</li> 	                                  
		                                          <p class="sub-header"></p>
		                                       </c:otherwise>
		                                   </c:choose>		                                 
		                                  <%--  <li>图书价格:&nbsp;&nbsp;${returns.borrowInfo.bookInfo.bookPrice}</li>		                                  
		                                     <p class="sub-header"></p> --%>
	                                   </ul>
	                                 </div>
	                              </li>
                    ...
                    <input type="button" class="btn btn-primary" id="confirm" name="back" value="确认" onclick="ReturnBook(this.id)" style="margin-left:10px;"/>
                    <input type="button" class="btn btn-primary" id="cancel" name="back" value="取消" onclick="ReturnBook(this.id)" style="margin-left:20px;"/>
                    <input type="button" class="btn btn-link" id="miss" name="back" value="图书丢失" onclick="BookMiss(${returns.borrowInfo.bookInfo.bookPrice})" style="margin-left:20px;margin-top:20px;"/>
				</div>
			<!-- </div>
		</div> -->
